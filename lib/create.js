...
//TODO: переименовать:)
function prepareEntityData(entities, levels, techs, options) {
    options || (options = {});
    techs || (techs = []);
...
module.exports = function create(entities, levels, techs, options) {
    ...
    return Promise.all(createRes.map(function(item) {
        var template = item.options.fileContent || getTemplate(item.tech, item.levelOptions),
            content = typeof template === 'string' ?
            template : template(item.entity, bemNaming(item.levelOptions.naming));

        return createEntity(item.path, content, item.levelOptions);
    }));
};
